...
<div class="app_launch_header">
  <form class="app_launch_form_filter app_form" action="<?= url("/app/filter"); ?>" method="post">
    ...
    <select name="status">
      ...
      <option
        value="paid" <?= (!empty($filter->status) && $filter->status == "paid" ? "selected" : ""); ?>><?= ($type == 'income' ? "Receitas recebidas" : "Despesas pagas"); ?></option>
      <option
        value="unpaid" <?= (!empty($filter->status) && $filter->status == "unpaid" ? "selected" : ""); ?>><?= ($type == 'income' ? "Receitas não recebidas" : "Despesas não pagas"); ?></option>
    </select>
